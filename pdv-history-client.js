 ...
 *   - AI 응답을 받을 때마다: const r = await interceptPdvTags(aiText, {svc, ipv6, authToken});
 ...
 * 최소화해야 한다.
 */
async function queryPdvScope({ svc, ipv6, authToken, scope, period, reason, resumeContext }) {
  const stored = _loadPdvConsent(scope);
  const body = {
    query: {
      svc, ipv6, scope: [scope], period: _periodToRange(period), auth_token: authToken,
      purpose: reason || '',
 ...
 */
async function interceptPdvTags(responseText, { svc, ipv6, authToken, resumeContext } = {}) {
  ...
  for (const m of matches) {
    ...
    const r = await queryPdvScope({
      svc, ipv6, authToken, scope: params.scope, period: params.period,
      reason: params.reason, resumeContext,
    });
